...
class Simulation:
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                self.particles.append(
                    Particle2D(
                        position=(1.41* i / (self.rest_density), 4+ 1.41* j / (self.rest_density)),
                        velocity=(0.0, 0.0),
                        mass=1.0,         
                        density=0.0,
                        near_density=0.0
    ...
    def calculate_near_densities(self):
        for i, particle in enumerate(self.particles):
            ...
            for j in neighbor_indices:
                ...
                if distance < self.smoothing_radius:
                    influence = near_density_kernel(self.smoothing_radius, distance)
                    particle.near_density += other.mass * influence
    ...
    def calculate_pressure_forces(self):
        for i, particle in enumerate(self.particles):
            ...
            if particle.density > 0:
                particle.velocity = (
                    particle.velocity[0] + pressure_gradient[0] * self.time_step / particle.density,
                    particle.velocity[1] + pressure_gradient[1] * self.time_step / particle.density
                )

    def density_to_pressure(self, density):
        return self.pressure_multiplier * (density - self.rest_density)

    def shared_pressure(self, density_a, density_b):
        return (self.density_to_pressure(density_a) + self.density_to_pressure(density_b)) / 2.0
    ...
    def calculate_viscosity_forces(self):
        for i, particle in enumerate(self.particles):
            ...
            if particle.density > 0:
                particle.velocity = (
                    particle.velocity[0] + viscosity_force[0] * self.time_step * self.viscosity_constant / particle.density,
                    particle.velocity[1] + viscosity_force[1] * self.time_step * self.viscosity_constant / particle.density
                )
    ...
    def calculate_external_forces(self):
        for particle in self.particles:
            if particle.density > 0:
                particle.velocity = (
                    particle.velocity[0],
                    particle.velocity[1] + self.gravitional_constant * self.time_step / particle.density
                )
